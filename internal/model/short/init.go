...
type ShortDaoInterface interface {
	...
	GetList(page, pageSize int, shortCode string) (urls []Short, count int64, err error)
...
func newShortDao(db *gorm.DB, model any, cache *redis.Redis, bid string) *shortDao {
	return &shortDao{db: db, model: model, cache: cache, bid: bid}
}

// GetShortUrl 获取短链接
func (d *shortDao) GetShortUrl(code string) (url *Short, err error) {
	//u := &Short{}
	data, err := d.cache.Get(code)
	...
	if err == redis.Nil || len(data) == 0 {
		...
		err = d.cache.Setex(code, data, 360)
		if err != nil {
			return nil, err
		}
	}
...
func (d *shortDao) UpdateByCode(code string, data Short) (err error) {
	...
	_, _ = d.cache.Del(code)
	return
}

// GetList 获取列表
func (d *shortDao) GetList(page, pageSize int, shortCode string) (urls []Short, count int64, err error) {
	tx := d.db.Model(d.model).Table(d.bid)
	err = tx.Where("code like ?", "%"+shortCode+"%").Offset((page - 1) * pageSize).Limit(pageSize).Find(&urls).Error
	if err != nil {
		return nil, 0, err
	}
